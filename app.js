 ...
 * @LastEditTime: 2021-09-19 08:15:28
 */
...
const { usename, password, gyms } = require('./config');
const { login, bookByGym } = require('./src/utils/pupputeer');
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            while (!status && idx < gyms.length) {
                // 预定场地
                status = await bookByGym(page, gyms[idx], idx === 0)
                idx++
            }
            // 退出浏览器
            await browser.close()
